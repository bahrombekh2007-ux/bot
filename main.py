...
def convert_doc_to_docx(input_path, output_path):
    ...
    if platform.system() == "Windows":
        msg = (
            "DOC faylni ochib bo'lmadi. Iltimos, Windows Word yoki LibreOffice o'rnatilganligini tekshiring. "
...
def parse_txt(file_path):
    """
    Parse TXT files with two possible formats:
    1) Questions separated by '#' lines, then question line, then lines starting with '+' (correct) and '-' (wrong).
    2) Questions start with '? ' and end with ' ?', then same option lines.
    """
    ...
    while i < n:
        ...
        if line == "#":
            i += 1
            # skip empty lines until we find question
            ...
            while i < n:
                ...
                if not opt_line:
                    i += 1
                    continue
                # stop at next '#' or next question pattern
                if opt_line == "#" or (opt_line.startswith('?') and opt_line.endswith('?')):
                    break
                if opt_line.startswith('+'):
                    correct = opt_line[1:].strip()
                    options.append(correct)
                elif opt_line.startswith('-'):
                    options.append(opt_line[1:].strip())
                else:
                    # unknown line, stop
                    break
                i += 1
        ...
        # Format 2: line starts and ends with '?'
        if line.startswith('?') and line.endswith('?'):
            # Extract question text (remove first '?' and last '?')
            question = line[1:-1].strip()
            # optionally remove numbering like "1. " but keep as is
            i += 1
            options = []
            correct = None
            while i < n:
...
def get_count_keyboard(total):
    ...
    for i, count in enumerate(options):
        label = f"📝 {count} ta" if count < total else f"📚 Hammasi ({count} ta)"
...
async def start(message: Message, state: FSMContext):
    ...
    msg = await message.answer(
        ...
        parse_mode="HTML", reply_markup=main_menu_keyboard(message.from_user.id)
    )
...
async def new_test(message: Message, state: FSMContext):
    ...
    msg = await message.answer(
        "📄 <b>YANGI TEST</b>\n━━━━━━━━━━━━━━━━━━━\n\n"
        "Test boshlash uchun DOCX, DOC yoki TXT formatdagi faylingizni yuboring 📎\n\n"
...
async def restart_previous_doc(message: Message, state: FSMContext):
    ...
    if user_id not in users or not users[user_id].get("uploaded_docs"):
        msg = await message.answer("❌ Oldingi DOCX topilmadi. Avval yangi test uchun DOCX yuboring!", reply_markup=main_menu_keyboard(user_id))
    ...
    for index, doc in enumerate(docs):
        ...
        buttons.append([InlineKeyboardButton(text=f"{index+1}. {title} ({count} ta)", callback_data=f"restart_doc_{index}")])
...
async def restart_doc(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text(
            f"📝 <b>TEST SONINI TANLANG</b>\n━━━━━━━━━━━━━━━━━━━\n\n📚 Mavjud savollar: <b>{total} ta</b>\n\n<i>Quyidagi variantlardan birini tanlang</i> 👇",
            parse_mode="HTML", reply_markup=get_count_keyboard(total)
        )
    except Exception:
...
async def help(message: Message):
    ...
    msg = await message.answer(
        ...
        "⚠️ <b>Muhim eslatma:</b>\n• Har bir javobdan keyin 'O'tkazib yuborish' tugmasini bosing\n"
...
async def test_result(message: Message):
    ...
    if user_id not in users or "results" not in users[user_id]:
        msg = await message.answer(
            ...
            "Siz hali test ishlamagansiz yoki natijalar o'chib ketgan.\n\n📄 Yangi test boshlang!",
            parse_mode="HTML", reply_markup=main_menu_keyboard(message.from_user.id)
        )
        await add_message(message.from_user.id, msg.message_id)
        return
    ...
    if not results:
        msg = await message.answer("📊 Hali natijalar mavjud emas", reply_markup=main_menu_keyboard(message.from_user.id))
        await add_message(message.from_user.id, msg.message_id)
        return
    text = "📊 <b>TEST NATIJALARI</b>\n━━━━━━━━━━━━━━━━━━━\n\n"
    for i, result in enumerate(results[-5:], 1):
        text += f"<b>{i}.</b> {result['date']}\n   📝 {result['total']} ta | ✅ {result['score']} ta\n   📈 {result['percentage']:.1f}% | {result['grade']}\n\n"
    text += "<i>Yangi test uchun DOCX/TXT yuboring</i> 📄"
    msg = await message.answer(text, parse_mode="HTML", reply_markup=main_menu_keyboard(message.from_user.id))
    await add_message(message.from_user.id, msg.message_id)
...
async def handle_doc(message: Message, state: FSMContext):
    ...
    if not (file_name.endswith(".docx") or file_name.endswith(".doc") or file_name.endswith(".txt")):
        msg = await message.answer("❌ <b>Faqat .docx, .doc yoki .txt fayl yuboring!</b>", parse_mode="HTML", reply_markup=main_menu_keyboard(message.from_user.id))
        await add_message(message.from_user.id, msg.message_id)
        return
    loading_msg = await message.answer("⏳ <b>Test yuklanmoqda...</b>\n━━━━━━━━━━━━━━━━━━━\n🔄 Fayl tahlil qilinmoqda...", parse_mode="HTML")
    ...
    try:
        ...
        if not os.path.exists(save_path):
            await clean_chat(message.from_user.id, message.chat.id)
            msg = await message.answer("❌ Fayl saqlanmadi. Iltimos yana urinib ko'ring.", reply_markup=main_menu_keyboard(message.from_user.id))
            await add_message(message.from_user.id, msg.message_id)
            return
        if ext == ".txt":
        ...
        await clean_chat(message.from_user.id, message.chat.id)
        msg = await message.answer(
            f"✅ <b>Test muvaffaqiyatli yuklandi!</b>\n━━━━━━━━━━━━━━━━━━━\n\n📚 Jami savollar: <b>{len(questions)} ta</b>\n📄 Fayl: <code>{doc.file_name}</code>\n\n<i>Test sonini tanlang...</i>",
            parse_mode="HTML",
        ...
        await clean_chat(message.from_user.id, message.chat.id)
        msg = await message.answer(f"❌ <b>Xatolik:</b> <code>{str(e)}</code>", parse_mode="HTML", reply_markup=main_menu_keyboard(message.from_user.id))
        await add_message(message.from_user.id, msg.message_id)
...
async def cancel_doc(callback: CallbackQuery, state: FSMContext):
    ...
    msg = await callback.message.answer("📄 Yangi test uchun DOCX/TXT yuboring", reply_markup=main_menu_keyboard(callback.from_user.id))
...
async def set_count(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text(
            f"📝 <b>TEST SONINI TANLANG</b>\n━━━━━━━━━━━━━━━━━━━\n\n📚 Mavjud savollar: <b>{total} ta</b>\n\n<i>Quyidagi variantlardan birini tanlang</i> 👇",
            parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=count_buttons)
        )
    except:
        msg = await callback.message.answer(
            f"📝 <b>TEST SONINI TANLANG</b>\n━━━━━━━━━━━━━━━━━━━\n\n📚 Mavjud savollar: <b>{total} ta</b>\n\n<i>Quyidagi variantlardan birini tanlang</i> 👇",
...
async def process_custom_count(message: Message, state: FSMContext):
    ...
    await clean_chat(message.from_user.id, message.chat.id)
    if user_id not in users:
        msg = await message.answer("❌ Avval test yuklang!", reply_markup=main_menu_keyboard(message.from_user.id))
        await add_message(message.from_user.id, msg.message_id)
        await state.clear()
        return
    try:
        ...
        if count < 1 or count > total:
            msg = await message.answer(f"❌ <b>Noto'g'ri son!</b>\n1 dan {total} gacha raqam kiriting", parse_mode="HTML", reply_markup=main_menu_keyboard(message.from_user.id))
            await add_message(message.from_user.id, msg.message_id)
            return
        await start_test(message, user_id, count, state)
    except ValueError:
        msg = await message.answer("❌ <b>Raqam kiriting!</b>\nMasalan: 10, 15, 20...", parse_mode="HTML", reply_markup=main_menu_keyboard(message.from_user.id))
        await add_message(message.from_user.id, msg.message_id)
...
async def start_test(message, user_id, count, state: FSMContext):
    ...
    msg = await message.answer(
        f"🚀 <b>TEST BOSHLANDI!</b>\n━━━━━━━━━━━━━━━━━━━\n\n📝 Jami: <b>{count} ta savol</b>\n⏱ Vaqt chegarasi yo'q\n🎲 Savollar aralash holda\n\n⚠️ <b>Muhim:</b> Javob berganingizdan keyin '<i>O'tkazib yuborish</i>' tugmasini bosing!\n\n<i>Omad!</i> 🍀",
        parse_mode="HTML"
    )
    await add_message(user_id, msg.message_id)
...
async def show_poll_results(chat_id, user_id, stopped=False):
    ...
    result_text = (
        ...
        f"📊 <b>Statistika:</b>\n• Jami savollar: <b>{total} ta</b>\n• Javob berilgan: <b>{answered} ta</b>\n"
        f"• O'tkazib yuborilgan: <b>{skipped} ta</b>\n\n✅ To'g'ri: <b>{score} ta</b>\n❌ Noto'g'ri: <b>{incorrect} ta</b>\n"
...
async def retry_poll_test(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_delete_message(callback.message.chat.id, callback.message.message_id)
    msg = await callback.message.answer(
        f"🔄 <b>TEST QAYTA BOSHLANDI!</b>\n━━━━━━━━━━━━━━━━━━━\n\n📝 Jami: <b>{data['total_test']} ta savol</b>\n"
...
async def show_poll_details(callback: CallbackQuery):
    ...
    for i, ans in enumerate(answers, 1):
        ...
        if len(ans["question"]) > 60:
            question_text += "..."
        details += f"<b>{i}.</b> {question_text}\n   {status} Siz: <i>{ans['user_answer']}</i>\n"
...
async def show_all_results(callback: CallbackQuery):
    ...
    for i, result in enumerate(results[-10:], 1):
        text += f"<b>{i}.</b> {result['date']}\n   📝 {result['total']} ta | ✅ {result['score']} ta\n   📈 {result['percentage']:.1f}% | {result['grade']}\n\n"
    msg = await callback.message.answer(text, parse_mode="HTML", reply_markup=main_menu_keyboard(user_id))
    await add_message(user_id, msg.message_id)
    await callback.answer()
